...
>
> *Оберните ваш кошелёк в 3 строки. GhostWallet останавливает плохие транзакции.*

**GhostWallet** — это детерминированный слой принуждения между намерением бота и транзакцией в блокчейне. Это аварийный выключатель между вашим автономным агентом для торговли / минтинга / DeFi и опустошённым кошельком.
...
GhostWallet — это развёртывание стандарта управления **Maelstrom Gate** для защиты кошелька — та же ментальная модель, применённая к крипто-подписчику: намерения проходят через детерминированную цепочку политик с журналом аудита.

GhostWallet остаётся самодостаточным с **нулевыми жёсткими зависимостями** от любых других пакетов Gate. Приёмник аудита и загрузчик политик — это протоколы, так что `gate-policy`, `gate-compliance` и другие могут подключаться, не касаясь самого кошелька. Вы можете запускать GhostWallet автономно всегда или встраивать его в более крупный стек управления агентами, когда будете готовы.
