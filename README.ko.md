...
>
> *3줄로 지갑을 감싸세요. GhostGate가 나쁜 트랜잭션을 차단합니다.*

**GhostGate**는 봇의 의도와 온체인 트랜잭션 사이에 있는 결정론적 집행 레이어입니다. 자율 거래 / 민팅 / DeFi 에이전트와 털린 지갑 사이에 자리잡는 서킷 브레이커입니다.
...
GhostGate는 **Maelstrom Gate** 거버넌스 표준의 지갑 보호 배포입니다 —— 암호 서명자에 적용된 동일한 사고 모델: 의도는 감사 추적과 함께 결정론적 정책 체인을 통해 흐릅니다.

GhostGate는 다른 어떤 Gate 패키지에 대해서도 **하드 의존성 없이** 자체 완결성을 유지합니다. 감사 싱크와 정책 로더는 프로토콜이므로, `gate-policy`, `gate-compliance` 등이 지갑 자체를 건드리지 않고 플러그인할 수 있습니다. GhostGate를 영원히 독립 실행형으로 운영하거나, 준비가 되면 더 큰 에이전트 거버넌스 스택에 구성할 수 있습니다.
